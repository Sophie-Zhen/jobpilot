...
| `src/jobpilot/cli.py` | CLI with argparse subparsers: `run`, `tailor`, `init-profile`, `story add/list/import/migrate/edit/delete/refine`, `status`, `update`, `feedback`, `gaps`. `tailor` runs a post-render **recruiter-scan evaluation** (`ats_score` + `evaluate_cv`) and prints the Yes/Maybe/No pile, ATS keyword gap, trajectory, and weak bullets between `=== EVALUATION ===` markers (so the bot relays it); `--no-eval` skips it for PDF-only speed. `digest` sorts referable jobs first and prepends a referral CTA to each card. |
